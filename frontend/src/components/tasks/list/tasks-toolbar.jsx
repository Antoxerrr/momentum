  ...
  return (
    <div className="flex justify-between my-5 h-[30px] items-center">
      ...
      <div className="flex flex-row-reverse items-center gap-2 select-none">
        {formActive ? (
          <RxCross2
            className="text-2xl cursor-pointer transition-colors duration-200 hover:text-danger-400 me-1"
            onClick={hideForm}
          />
        ) : (
          <GoPlus
            className="text-3xl cursor-pointer hover:text-primary-500 transition-colors duration-200"
            onClick={showForm}
          />
        )}
      </div>
    ...
    <>
      ...
      >
        <SelectItem
          ...
          startContent={tabsIcons[TASKS_LIST_TABS.CURRENT]}
        >
          Текущие
        </SelectItem>
        <SelectItem
          key={TASKS_LIST_TABS.UPCOMING}
          startContent={tabsIcons[TASKS_LIST_TABS.UPCOMING]}
        >
          Предстоящие
        </SelectItem>
        <SelectItem
          key={TASKS_LIST_TABS.ARCHIVED}
          startContent={tabsIcons[TASKS_LIST_TABS.ARCHIVED]}
        >
          Архив
        </SelectItem>
      </Select>
